<x-app-with-sidebar>
    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                <x-slot name="header">
                    <h3 class="text-lg font-medium">Form Tambah Tipe Pabrik</h3>
                </x-slot>

                <form action="{{ route('admin.factory-types.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf

                    <div class="space-y-6">
                        <x-form-group>
                            <x-slot name="label">Nama Tipe Pabrik</x-slot>
                            <x-text-input type="text" name="name" value="{{ old('name') }}" placeholder="e.g., Pabrik Beton, Pabrik Bata" required />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </x-form-group>

                        <x-form-group>
                            <x-slot name="label">Deskripsi</x-slot>
                            <x-textarea-input name="description" rows="4" placeholder="Deskripsi tipe pabrik...">{{ old('description') }}</x-textarea-input>
                        ...
                        <x-form-group>
                            ...
                            <input type="file" name="icon" accept="image/*,.svg" 
                                   class="block w-full text-sm text-gray-500 dark:text-gray-400
                                          ...
                                          hover:file:bg-primary-100
                                          dark:file:bg-primary-900 dark:file:text-primary-300">
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Icon kecil untuk tipe pabrik (SVG atau gambar kecil)</p>
                        ...
                        <x-form-group>
                            ...
                            <x-textarea-input name="specifications_template" rows="6" placeholder='{"grade": "K-300", "slump": "12cm", "strength": "30 MPa"}'></x-textarea-input>
