...
<x-app-with-sidebar>
    ...
    @if($stores->count() > 0)
        <!-- Bulk Actions -->
        <x-card class="mb-6">
            <form id="bulkActionForm" action="{{ route('admin.stores.bulk-action') }}" method="POST">
                @csrf
                ...
                <x-card>
                    <div class="flex items-start space-x-4">
                        <div class="flex-shrink-0 pt-1">
                            <input type="checkbox" name="store_ids[]" value="{{ $store->uuid }}" 
                                   class="store-checkbox rounded border-gray-300 text-primary-600 focus:ring-primary-500">
                        </div>
                        @if($store->logo)
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between mb-2">
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                                    <a href="{{ route('admin.stores.show', $store) }}" class="hover:text-primary-600">
                                        {{ $store->name }}
                                    </a>
                                </h3>
                            ...
                            <div class="flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400">
                                <span>{{ $store->locations->count() }} Lokasi</span>
                                <span>{{ $store->products->count() }} Produk</span>
                                <span>{{ $store->total_reviews }} Review</span>
                            </div>
                            @if($store->rejection_reason)
                                <p class="text-sm text-red-600 dark:text-red-400 mt-2">
                                    <strong>Alasan:</strong> {{ $store->rejection_reason }}
                                </p>
                            @endif
                        </div>
                        <div class="flex flex-col space-y-2">
                            ...
                            @if($store->status === 'pending')
                                ...
                                <button type="button" onclick="showRejectModal('{{ $store->uuid }}', '{{ $store->name }}')" class="w-full px-3 py-1.5 text-sm bg-red-600 text-white rounded hover:bg-red-700">
                                    Reject
                                </button>
                            @elseif($store->status === 'suspended')
                                <form action="{{ route('admin.stores.activate', $store) }}" method="POST" class="inline">
                                    @csrf
    ...
    <x-modal id="rejectModal" title="Reject Store">
        <form id="rejectForm" method="POST">
            @csrf
            <x-form-group label="Store Name" name="store_name">
                <p id="modalStoreName" class="text-gray-700 dark:text-gray-300"></p>
            </x-form-group>
