...
<x-app-with-sidebar>
    ...
    @if($products->count() > 0)
        <!-- Bulk Actions -->
        <x-card class="mb-6">
            <form id="bulkActionForm" action="{{ route('admin.products.bulk-action') }}" method="POST">
                @csrf
    ...
    <script>
        ...
        document.getElementById('bulkActionForm').addEventListener('submit', function(e) {
            ...
            let confirmMsg = '';
            if (action === 'approve') {
                confirmMsg = `Setujui ${checked.length} produk?`;
            } else if (action === 'reject') {
                confirmMsg = `Tolak ${checked.length} produk?`;
            } else if (action === 'delete') {
                confirmMsg = `Hapus permanen ${checked.length} produk? Tindakan ini tidak dapat dibatalkan!`;
            }
