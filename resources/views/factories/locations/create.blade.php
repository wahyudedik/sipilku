<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                <x-slot name="header">
                    <h3 class="text-lg font-medium">Form Tambah Lokasi</h3>
                </x-slot>

                <form action="{{ route('factories.locations.store', $factory) }}" method="POST">
                    @csrf

                    <div class="space-y-6">
                        <x-form-group label="Nama Lokasi" name="name">
                            <x-text-input type="text" name="name" value="{{ old('name') }}" placeholder="Kantor Pusat / Pabrik Utama" />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </x-form-group>

                        <x-form-group label="Alamat Lengkap *" name="address" required>
                            <x-textarea-input name="address" rows="3" required>{{ old('address') }}</x-textarea-input>
                            <x-input-error :messages="$errors->get('address')" class="mt-2" />
                        </x-form-group>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <x-form-group label="Kota *" name="city" required>
                                <x-text-input type="text" name="city" value="{{ old('city') }}" required />
                                <x-input-error :messages="$errors->get('city')" class="mt-2" />
                            </x-form-group>

                            <x-form-group label="Provinsi *" name="province" required>
                                <x-text-input type="text" name="province" value="{{ old('province') }}" required />
                                <x-input-error :messages="$errors->get('province')" class="mt-2" />
                            </x-form-group>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <x-form-group label="Kode Pos" name="postal_code">
                                <x-text-input type="text" name="postal_code" value="{{ old('postal_code') }}" />
                                <x-input-error :messages="$errors->get('postal_code')" class="mt-2" />
                            </x-form-group>

                            <x-form-group label="Latitude" name="latitude">
                                <x-text-input type="number" id="location_latitude" name="latitude" value="{{ old('latitude') }}" step="any" />
                                <x-input-error :messages="$errors->get('latitude')" class="mt-2" />
                            </x-form-group>

                            <x-form-group label="Longitude" name="longitude">
                                <x-text-input type="number" id="location_longitude" name="longitude" value="{{ old('longitude') }}" step="any" />
                        ...
                        <!-- Google Maps Picker -->
                        <div>
                            <x-form-group label="Pilih Lokasi di Peta">
                                <div class="space-y-2">
