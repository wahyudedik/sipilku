                    ...
                    @foreach($locations as $location)
                        <x-card>
                            <div class="flex items-start justify-between mb-4">
                                ...
                                <div class="flex space-x-2">
                                    <a href="{{ route('factories.locations.edit', [$factory, $location]) }}">
                                        <x-button variant="secondary" size="sm">Edit</x-button>
                                    </a>
                                    <form action="{{ route('factories.locations.destroy', [$factory, $location]) }}" method="POST" onsubmit="return confirm('Hapus lokasi ini?')">
                                        @csrf
                                        @method('DELETE')
            ...
            @else
                <x-card>
                    <div class="text-center py-12">
                        <p class="text-gray-500 dark:text-gray-400 mb-4">Belum ada lokasi pabrik.</p>
                        <a href="{{ route('factories.locations.create', $factory) }}">
