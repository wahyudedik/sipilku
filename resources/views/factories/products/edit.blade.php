...
<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                <x-slot name="header">
                    <h3 class="text-lg font-medium">Form Edit Produk</h3>
                </x-slot>

                <form action="{{ route('factories.products.update', [$factory, $product]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                            ...
                            <div class="space-y-4">
                                @if(count($productCategories) > 0)
                                    <x-form-group label="Kategori Produk" name="product_category">
                                        <x-select-input name="product_category">
                                            <option value="">Pilih Kategori</option>
                                            @foreach($productCategories as $key => $label)
                                                <option value="{{ $key }}" {{ old('product_category', $product->product_category) === $key ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </x-select-input>
                                ...
                                <x-form-group label="Nama Produk *" name="name" required>
                                    <x-text-input type="text" name="name" value="{{ old('name', $product->name) }}" required />
                                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                                </x-form-group>

                                <x-form-group label="Deskripsi" name="description">
                                    <x-textarea-input name="description" rows="4">{{ old('description', $product->description) }}</x-textarea-input>
                                    <x-input-error :messages="$errors->get('description')" class="mt-2" />
                                </x-form-group>

                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <x-form-group label="SKU" name="sku">
                                        <x-text-input type="text" name="sku" value="{{ old('sku', $product->sku) }}" />
                                        <x-input-error :messages="$errors->get('sku')" class="mt-2" />
                                    </x-form-group>

                                    <x-form-group label="Kode Produk Pabrik" name="code">
                                        <x-text-input type="text" name="code" value="{{ old('code', $product->code) }}" />
                            ...
                            <div class="space-y-4">
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <x-form-group label="Harga Normal *" name="price" required>
                                        <x-text-input type="number" name="price" value="{{ old('price', $product->price) }}" step="0.01" min="0" required />
                                        <x-input-error :messages="$errors->get('price')" class="mt-2" />
                                    </x-form-group>

                                    <x-form-group label="Harga Diskon" name="discount_price">
                                        <x-text-input type="number" name="discount_price" value="{{ old('discount_price', $product->discount_price) }}" step="0.01" min="0" />
                                    ...
                                    <x-form-group label="Unit Utama *" name="unit" required>
                                        <x-select-input name="unit" required>
                                            ...
                                            <option value="m2" {{ old('unit', $product->unit) === 'm2' ? 'selected' : '' }}>M²</option>
                                            <option value="m3" {{ old('unit', $product->unit) === 'm3' ? 'selected' : '' }}>M³</option>
                                            ...
                                            <option value="mobil" {{ old('unit', $product->unit) === 'mobil' ? 'selected' : '' }}>Mobil</option>
                                            <option value="kubik" {{ old('unit', $product->unit) === 'kubik' ? 'selected' : '' }}>Kubik</option>
                                        </x-select-input>
                                        ...
                                        @if($product->available_units && count($product->available_units) > 0)
                                            @foreach($product->available_units as $unit)
                                                <div class="flex gap-2">
                                                    <x-text-input type="text" name="available_units[]" value="{{ $unit }}" class="flex-1" />
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <x-form-group label="Stok" name="stock">
                                    <x-text-input type="number" name="stock" value="{{ old('stock', $product->stock) }}" min="0" />
                                ...
                                <x-form-group label="Minimal Pemesanan" name="min_order">
                                    <x-text-input type="number" name="min_order" value="{{ old('min_order', $product->min_order) }}" min="1" />
                            ...
                            <div>
                                ...
                                <div class="space-y-4">
                                    <x-form-group label="Grade" name="quality_grade[grade]">
                                        <x-select-input name="quality_grade[grade]">
                                            ...
                                            @foreach($typeConfig['quality_grades'] as $value => $label)
                                                <option value="{{ $value }}" {{ old('quality_grade.grade', $product->quality_grade['grade'] ?? '') === $value ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </x-select-input>
                                    ...
                                    <x-form-group label="Nilai Grade" name="quality_grade[value]">
                                        <x-text-input type="text" name="quality_grade[value]" value="{{ old('quality_grade.value', $product->quality_grade['value'] ?? '') }}" />
                                    </x-form-group>

                                    <x-form-group label="Deskripsi Grade" name="quality_grade[description]">
                                        <x-textarea-input name="quality_grade[description]" rows="2">{{ old('quality_grade.description', $product->quality_grade['description'] ?? '') }}</x-textarea-input>
                                    </x-form-group>
                                </div>
                            </div>
                        @endif
                            ...
                            @if($product->images && count($product->images) > 0)
                                <div class="grid grid-cols-4 gap-4 mb-4">
                                    @foreach($product->images as $image)
                                        <div class="relative">
                                            <img src="{{ Storage::url($image) }}" alt="Product image" class="w-full h-24 object-cover rounded">
                                        </div>
                                    @endforeach
                                </div>
                                <p class="text-xs text-gray-500 mb-2">Gambar saat ini. Upload gambar baru akan mengganti semua gambar.</p>
                            @endif
                            <x-form-group label="Upload Gambar Baru (Maks 10)" name="images">
                        ...
                        <div>
                            ...
                            @if(count($specificationsTemplate) > 0 && (!$product->specifications || count($product->specifications) === 0))
                            ...
                            <div id="specifications-container" class="space-y-2">
                                ...
                                @else
                                    <div class="flex gap-2">
                                        <x-text-input type="text" name="spec_key[]" placeholder="Nama" class="flex-1" />
                                        <x-text-input type="text" name="spec_value[]" placeholder="Nilai" class="flex-1" />
                                        <button type="button" onclick="removeSpec(this)" class="px-3 py-2 bg-red-600 text-white rounded hover:bg-red-700">Hapus</button>
                                    </div>
                                @endif
                            </div>
