    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                ...
                <form action="{{ route('store-reviews.update', [$store, $review]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="space-y-6">
                        <!-- Overall Rating -->
                            ...
                            <div class="space-y-3">
                                <div>
                                    ...
                                    <select name="ratings_breakdown[product_quality]" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800">
                                        ...
                                        @for($i = 5; $i >= 1; $i--)
                                            <option value="{{ $i }}" {{ old('ratings_breakdown.product_quality', $review->ratings_breakdown['product_quality'] ?? '') == $i ? 'selected' : '' }}>{{ $i }} Bintang</option>
                                        @endfor
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <select name="ratings_breakdown[service]" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800">
                                        <option value="">Pilih rating</option>
                                        @for($i = 5; $i >= 1; $i--)
                                            <option value="{{ $i }}" {{ old('ratings_breakdown.service', $review->ratings_breakdown['service'] ?? '') == $i ? 'selected' : '' }}>{{ $i }} Bintang</option>
                                        @endfor
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <select name="ratings_breakdown[price]" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800">
                                        <option value="">Pilih rating</option>
                                        @for($i = 5; $i >= 1; $i--)
                                            <option value="{{ $i }}" {{ old('ratings_breakdown.price', $review->ratings_breakdown['price'] ?? '') == $i ? 'selected' : '' }}>{{ $i }} Bintang</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                        ...
                        <div>
                            ...
                            <textarea name="comment" id="comment" rows="5" class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 focus:border-primary-500 focus:ring-primary-500" placeholder="Bagikan pengalaman Anda dengan toko ini...">{{ old('comment', $review->comment) }}</textarea>
