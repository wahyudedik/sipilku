<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Beri Review untuk {{ $factory->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                ...
                <form action="{{ route('factory-reviews.store', $factory) }}" method="POST">
                    @csrf
                    <div class="space-y-6">
                        <!-- Overall Rating -->
                        <div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div class="flex items-center space-x-2" id="productQualityContainer">
                                        @for($i = 5; $i >= 1; $i--)
                                            <input type="radio" name="ratings_breakdown[product_quality]" value="{{ $i }}" id="product_quality{{ $i }}" class="hidden" {{ old('ratings_breakdown.product_quality') == $i ? 'checked' : '' }}>
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center space-x-2" id="deliveryQualityContainer">
                                        @for($i = 5; $i >= 1; $i--)
                                            <input type="radio" name="ratings_breakdown[delivery_quality]" value="{{ $i }}" id="delivery_quality{{ $i }}" class="hidden" {{ old('ratings_breakdown.delivery_quality') == $i ? 'checked' : '' }}>
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center space-x-2" id="serviceQualityContainer">
                                        @for($i = 5; $i >= 1; $i--)
                                            <input type="radio" name="ratings_breakdown[service_quality]" value="{{ $i }}" id="service_quality{{ $i }}" class="hidden" {{ old('ratings_breakdown.service_quality') == $i ? 'checked' : '' }}>
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center space-x-2" id="priceContainer">
                                        @for($i = 5; $i >= 1; $i--)
                                            <input type="radio" name="ratings_breakdown[price]" value="{{ $i }}" id="price{{ $i }}" class="hidden" {{ old('ratings_breakdown.price') == $i ? 'checked' : '' }}>
                        ...
                        <div>
                            ...
                            <textarea name="comment" id="comment" rows="5" class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 focus:border-primary-500 focus:ring-primary-500" placeholder="Bagikan pengalaman Anda dengan pabrik ini...">{{ old('comment') }}</textarea>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Handle overall rating star click
            ...
            // Handle overall rating input change
