    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                ...
                <form action="{{ route('stores.products.update', [$store, $product]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <x-form-group>
                                        ...
                                        <x-text-input type="text" name="brand" value="{{ old('brand', $product->brand) }}" />
                                ...
                                <x-form-group>
                                    ...
                                    <select name="store_category_id" class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900">
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->uuid }}" {{ old('store_category_id', $product->store_category_id) === $category->uuid ? 'selected' : '' }}>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div>
                                ...
                                <div class="grid grid-cols-4 gap-4 mb-4">
                                    @foreach($product->images as $image)
                                        <div class="relative">
                                            ...
                                            <button type="button" onclick="removeImage(this)" class="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1">
                            ...
                            @endif
                            <x-form-group>
                                <x-slot name="label">Tambah Gambar Baru (Maks 10)</x-slot>
                        ...
                        <!-- Specifications -->
                        <div>
                            <h4 class="text-md font-medium mb-4">Spesifikasi</h4>
                            <div id="specifications-container" class="space-y-2">
                                @if($product->specifications && count($product->specifications) > 0)
                                    @foreach($product->specifications as $key => $value)
                                        <div class="flex gap-2">
                                            <x-text-input type="text" name="spec_key[]" value="{{ $key }}" placeholder="Nama" class="flex-1" />
                                            <x-text-input type="text" name="spec_value[]" value="{{ $value }}" placeholder="Nilai" class="flex-1" />
